...
input ENUM_TIMEFRAMES InpTimeFrame = PERIOD_M20; // Range time frame
//input int InpTimeFrame = 30;                    // Range time frame minutes
// input int InpAtrPeriod = 1000;                  // ATR Period
input ENUM_TIMEFRAMES InpFastEntry = PERIOD_M1;
...
bool InpImportance_all = InpImportance == IMPORTANCE_ALL;
...
input group "========= Time =========";
 ...
 bool InpDaylightSaving = false; // DST zone
...
input group "Sesh";
input int InpFirstHour=9, InpSecondHour=14;
int firstHour, secondHour;
datetime holiDay;
int totalBarsEvent;
bool IsNewsEvent()
{
  if (!IsNewBar(PERIOD_M1, barsTotal2) || upperLine > 0 || lowerLine > 0 || PositionCount() > 0)
    return false;
  ...
  datetime iDay = iTime(Symbol(), PERIOD_M1, 0);
  ...
  for (int i = amount - 1; i >= 0; i--)
  {
    ...
    if (event.type == CALENDAR_TYPE_HOLIDAY && (value.time > iDay && value.time < iDay+PeriodSeconds(PERIOD_D1))) {holiDay=iDay;return false;} 
     ...
     if(time.min == 0 && (time.hour == firstHour || time.hour == secondHour))
      {
       return true;
      }
      else
...
double AtrValue()
{
  int atrHandle = iATR(Symbol(), InpTimeFrame, atrPeriod);
  double atrBuffer[];
...
double GetPositionSize()
{
  ...
  double RiskMultiplier = InpFastExitHedge ? 1 + PositionCount()*0.1 : 1;
  if (lastDirection == 1)
  {
  ...
  double  PS1 = PeriodSeconds(InpTimeFrame);
  double  PS2 = PeriodSeconds(InpFastEntry > PERIOD_M1? InpFastEntry:InpTimeFrame);
  double fastEntry = PS1/PS2;
datetime calculatedZoneTime;
double upperEntry, lowerEntry;
void CalculateZone()
{
  if(PositionCount() == 0 && TimeCurrent() > calculatedZoneTime+PeriodSeconds(InpFastEntry > PERIOD_M1?InpFastEntry:InpTimeFrame) && upperLine != 0 && lowerLine != 0) {
     upperLine = 0;
     lowerLine = 0;
     upperEntry=0;
     lowerEntry=0;
...
void ShowInfo()
{
  if (InpShowInfo)
    Comment("Server time: ", TimeCurrent(), "\n",
            ...
            "fast enty: ", fastEntry, "\n",
            "PeriodSeconds(InpTimeFrame): ", PS1, "\n",
            "PeriodSeconds(InpFastEntry): ", PS2, "\n",
