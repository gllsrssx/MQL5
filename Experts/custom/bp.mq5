...
input double multiplier = 4.0; // multiplier
...
void OnTick()
{
   ...
   if(PositionsTotal() == 0 && CheckMaxDDThreshold(maxLossPercentDay) && isFirstCandle && !isHedge && !cascade && atrSpike(atrValue(1),atrValue(7200),multiplier)){
      if(DirectionLong()){
         entryPrice = ask;
         stopPrice = ask - atrValue(1);
...
void CloseOlderTrades()
{
    ...
    ulong lastTicket = PositionGetTicket(0);
