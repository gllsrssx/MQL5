...
input bool InpMartinGale = true; // martin
...
input int InpFastEntry = 60; // fast entry minutes (0=off)
input double InpZoneTimeDivider = 60; // Minutes to enter
...
input bool InpShowInfo = false;       // Show Info
input bool InpShowLines = false;      // Show Range
...
double highestBalance = AccountInfoDouble(ACCOUNT_BALANCE);
double Volume(double slDistance)
{
   ...
   //double martingaleFactor =1;
    

    // Calculate the risk amount in money
    double riskMoney = highestBalance * InpRisk * 0.01;
if (InpMartinGale && InpMaxHedges == 1 && balance < highestBalance) riskMoney+=highestBalance-balance;
    // Calculate the money value for the step in lots
...
int PositionCount()
{
  ...
  for (int i = PositionsTotal() - 1; i >= 0; i--)
  {
    ...
    if (PositionGetString(POSITION_SYMBOL) != Symbol() || PositionGetInteger(POSITION_MAGIC) != InpMagicNumber)
      continue;
    if(PositionGetDouble(POSITION_VOLUME)==maxVol)continue;
    count++;
  }
  return count;
}
...
void TakeTrade()
{
  ...
  double lots = Volume(diff);
      double dist = 0;
      if(InpMartinGale && InpMaxHedges==1) dist = upperLine-lowerLine;
  if (tick.ask >= (posC == 0 ? upperEntry : upperLine) && lastDirection != 1 && tick.bid <= (posC == 0 ? upperEntry+diff : upperLine+diff)) {
    if(InpMaxHedges > 0 && posC >= InpMaxHedges){CloseTrades(); return;}
    while (lots > 0){
      trade.Buy(NormalizeDouble(lots > maxlot ? maxlot : lots, 2),NULL,0,tick.ask-dist,tick.ask+dist,lastNewsEvent);
      lots -= maxlot;
      if(posC==0&&PositionCount()==1){upperLine=upperEntry;lowerLine=upperEntry-diff;}
    }
    Print(lastNewsEvent);
  }
  if (tick.bid <= (posC == 0 ? lowerEntry : lowerLine) && lastDirection != -1 && tick.ask >= (posC == 0 ? upperEntry-diff : upperLine-diff)) {
    if(InpMaxHedges > 0 && posC >= InpMaxHedges){CloseTrades(); return;}
    while (lots > 0){
      trade.Sell(NormalizeDouble(lots > maxlot ? maxlot : lots, 2),NULL,0,tick.bid+dist,tick.bid-dist,lastNewsEvent);
      lots -= maxlot;
      if(posC==0&&PositionCount()==1){lowerLine=lowerEntry;upperLine=lowerEntry+diff;}
    }
    Print(lastNewsEvent);
  }
}
...
void CloseTrades()
{
  ...
  bool tradeLoss = ((AccountInfoDouble(ACCOUNT_EQUITY) <= AccountInfoDouble(ACCOUNT_BALANCE) * InpStopOut * 0.01 && InpStopOut > 0));
  ...
  if(InpMartinGale && InpMaxHedges==1){
    tradeWin = AccountInfoDouble(ACCOUNT_EQUITY) >= highestBalance * (1 + InpRisk * InpRiskReward * 0.01);
   tradeLoss=false;
   tradeWin=false;
  }
