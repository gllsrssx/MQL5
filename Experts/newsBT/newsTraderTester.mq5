...
input double InpATRMultiplier = 1.0;             // ATR Trail SL Multiplier (0 = disabled)
...
double AtrValue()
{
   ...
   if (InpATRMultiplier == 0)
      atrValue = NormalizeDouble(priceArray[0] * 4, Digits());
   ;
...
void Trail()
{
   ...
   for (int i = total - 1; i >= 0; i--)
   {
      ...
      // continue if the open time is the same as the current bar open time
      datetime openTime;
      ...
      if (openTime == iTime(Symbol(), InpTimeFrame, 0))
         continue;
...
void CloseAfterOneHour()
{
   ...
   for (int i = total - 1; i >= 0; i--)
   {
      ...
      if (TimeCurrent() - openTime > PeriodSeconds(PERIOD_H1))
      {
