...
double Volume(string symbol, int volDivider = 1)
{
   ...
   double riskMoney = balance * InpRisk / volDivider;
...
// Trade Tester
economicNews nieuwsTest[];
void TradeTester()
{
   datetime candleOpen = iTime(Symbol(), PERIOD_M5, 0);

   int amount = ArraySize(nieuwsTest);
   for (int i = amount - 1; i >= 0; i--)
   {

      MqlCalendarEvent event = nieuwsTest[i].event;
      MqlCalendarValue value = nieuwsTest[i].value;
      MqlCalendarCountry country = nieuwsTest[i].country;
