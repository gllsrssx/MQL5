...
input group "========= Extra =========";
 bool InpFastExitHedge = false; // fast exit
input bool InpHedgeMore = false; 
...
double GetPositionSize()
{
  ...
  double RiskMultiplier = InpHedgeMore ? NormalizeDouble(MathExp(-posC / (20 *InpRiskReward)),2) : 1;//InpFastExitHedge ? 1 + (posC*0.1) : 
  //Print(RiskMultiplier);
  RiskMultiplierString = (string)RiskMultiplier;
  if (lastDirection == 1)
  {
    return NormalizeDouble((((((InpRiskReward + 1) / InpRiskReward) * openBuyLots - openSellLots) + baseLots) * RiskMultiplier),2);
  }
  else if (lastDirection == -1)
  {
    return NormalizeDouble((((((InpRiskReward + 1) / InpRiskReward) * openSellLots - openBuyLots) + baseLots) * RiskMultiplier),2);
  }
  else
  {
    return baseLots;
