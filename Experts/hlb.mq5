...
void OnTick()
{
  ...
  if (cntBuy==0 && high!=0 && previousTick.ask < high && currentTick.ask >= high && CheckIndexFilter(highIndex) && CheckSizeFilter()){
    ...
    if(!NormalizePrice(sl)){Print("Failed to normalize stop loss price"); return;}
    if(!NormalizePrice(tp)){Print("Failed to normalize take profit price"); return;}

    trade.PositionOpen(Symbol(), ORDER_TYPE_BUY, InpStopLoss>0?Volume(InpStopLoss):InpLots, currentTick.ask, sl, tp, "HighLowBreakout");
  }
  ...
  if (cntSell==0 && low!=0 && previousTick.bid > low && currentTick.bid <= low && CheckIndexFilter(lowIndex) && CheckSizeFilter()){
    ...
    trade.PositionOpen(Symbol(), ORDER_TYPE_SELL, InpStopLoss>0?Volume(InpStopLoss):InpLots, currentTick.bid, sl, tp, "HighLowBreakout");
  }

  //update stop loss
  if(InpTrailingSL && InpStopLoss > 0){
    UpdateStopLoss(InpStopLoss * Point());
  }
...
void DrawObjects(){
  ...
  ObjectSetString(NULL, "text", OBJPROP_TEXT, "Bars: " + (string) InpBars + 
                                              ...
                                              " High Index: " + (string) highIndex +
                                              " Low Index: " + (string) lowIndex +
                                              " Size: " + DoubleToString((high - low) / Point(), 0));
...
bool CountOpenPositions(int &cntBuy, int &cntSell){
  ...
  for(int i=total-1; i>=0; i--){
    ...
    long magic;
    if(!PositionGetInteger(POSITION_MAGIC, magic)){Print("Failed to get magic number"); return false;}
    if(magic == InpMagic){
      long type;
...
void UpdateStopLoss(double slDistance){
  ...
  {
    ulong ticket = PositionGetTicket(i);
    if(ticket <=0){Print("Failed to get ticket"); return ;}
    if(!PositionSelectByTicket(ticket)){Print("Failed to select position by ticket"); return ;}
    ulong magic;
    if(!PositionGetInteger(POSITION_MAGIC, magic)){Print("Failed to get position magic"); return ;}
    if(magic == InpMagic)
    {
      long type;
      ...
      if(!PositionGetDouble(POSITION_SL, currSL)){Print("Failed to get stop loss"); return ;}
      if(!PositionGetDouble(POSITION_TP, currTP)){Print("Failed to get take profit"); return ;}
      ...
      if(!NormalizePrice(newSL)){Print("Failed to normalize stop loss price"); return ;}
      if((newSL*n) < (currSL*n) || NormalizeDouble(MathAbs(newSL-currSL), Digits()) < Point()){continue;}
      //check for stop level
      long level = SymbolInfoInteger(Symbol(), SYMBOL_TRADE_STOPS_LEVEL);
      if(level != 0 && MathAbs(currPrice - newSL) <= level * Point()){Print("Stop level violation"); continue;}
